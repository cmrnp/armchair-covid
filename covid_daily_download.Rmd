...
if (file.exists(rds_file)) {
  warning("overwriting existing Rds file!")
}
```
...
Most recent observations by state:

```{r}
max(covid19_vic$date)
max(covid19_nsw$date - 1)
max(covid19_act$date)
max(covid19_nz$`Report Date`)
```
