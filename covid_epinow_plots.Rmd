...
pred_int %>%
  ...
  geom_point(aes(y = daily_confirmed), colour = "chartreuse4", pch = 1, stroke = 1, alpha = 0.8) +
  expand_limits(y = 0) +
  ...
  theme(panel.grid.minor = element_blank(),
        ...
        legend.position = "off") +
  labs(x = "Notification date", y = "Daily confirmed cases excluding overseas",
       title = "GAM model for Covid cases in selected Australian states and New Zealand",
       caption = glue("Estimated using a negative binomial Generalised Additive Model, using the mgcv R package.\nData from github.com/pappubahry and NZ Health.\nBands represent 20%, 50%, 90% parametric bootstrap prediction intervals.\nPlot created {Sys.Date()} with most recent cases from {max(covid19_aus_munged$date)}. Contact: twitter.com/camjpatrick"))
```
...
brm_fit_empty <-
  brm(
    ...
    prior = c(prior(normal(log_mean, log_mean), class = Intercept),
              ...
              prior(normal(0, 10*log_sd), class = sds),
...
pred_int_brm %>%
  ...
  labs(x = "Notification date", y = "Daily confirmed cases excluding overseas",
       ...
       caption = glue("Estimated using a negative binomial Generalised Additive Model, using the brms R package.\nData from github.com/pappubahry and NZ Health.\nBands represent 20%, 50%, 90% posterior prediction intervals.\nPlot created {Sys.Date()} with most recent cases from {max(covid19_aus_munged$date)}. Contact: twitter.com/camjpatrick"))
```
...
est_cases %>%
  ...
  geom_point(aes(y = confirm), pch = 1, stroke = 1, alpha = 0.8) +
  ...
  labs(x = "Notification date", y = "Daily confirmed cases excluding overseas",
       ...
       caption = glue("Estimated using EpiNow2 R package using data from github.com/pappubahry and NZ Health.\nBands represent 20%, 50%, 90% posterior prediction intervals.\nPlot created {Sys.Date()} with most recent cases from {max(case_data$date)}. Contact: twitter.com/camjpatrick"))
```
...
est_R %>%
  ...
  geom_vline(xintercept = max(case_data$date), linetype = "dashed", colour = "grey50") +
  ...
  theme(panel.grid.minor = element_blank(),
        plot.title.position = "plot",
        legend.position = "off") +
  labs(x = "Notification date", y = "Estimated effective reproduction number (Reff)",
       title = "Estimated Reff in selected Australian states and New Zealand",
       caption = glue("Estimated using EpiNow2 R package. Data from github.com/pappubahry and NZ Health.\nBands represent 20%, 50%, 90% posterior credible intervals.\nPlot created {Sys.Date()} with most recent cases from {max(case_data$date)}. Contact: twitter.com/camjpatrick"))
```
